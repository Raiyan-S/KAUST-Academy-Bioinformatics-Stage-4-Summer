...
top_gene2 <- "SOCS2"
# counts for the top gene
top_gene_counts2 <- GetAssayData(pseudo_bulk, 
                                assay = "RNA", 
                                layer = "counts")[top_gene2, ]
# plot expression of the top gene across the clusters and sample groups
top_gene_df2 <- data.frame(Expression = top_gene_counts2,
                          Cluster = pseudo_bulk$Cluster,
                          SampleGroup = pseudo_bulk$SampleGroup,
                          SampleName = pseudo_bulk$SampleName)
ggplot(top_gene_df2, aes(x = SampleName, y = log(Expression)+1, color = SampleGroup
)) +
  geom_point() +
  xlab("Cluster") +
...
FeaturePlot(seurat_object, features = top_gene2, split.by = "SampleGroup")
