...
setwd("~/Desktop/Course_Materials/Week3_Single-cell_RNAseq/course_single_cell")
...
# We could apply all 3 filters together to get a vector of cell
...
# Create a named vector of directories for each sample
# YOUR CODE HERE
all_samples <- sampleinfo$Sample[]

# Read in the cellranger matrices for all samples
# YOUR CODE HERE
all_list_of_files <- file.path("Data/CellRanger_Outputs/",
                           all_samples,
                           "/outs/filtered_feature_bc_matrix")
names(all_list_of_files) <- sampleinfo$SampleName[]

# Create the seurat object
# YOUR CODE HERE
all_expression_matrix <- Read10X(data.dir = all_list_of_files)

# Create a new seurat object with the combined expression matrix
all_multi_seurat_object <- CreateSeuratObject(counts = all_expression_matrix, min.cells = 1)
all_multi_seurat_object


# Sample group and sample name information to the metadata
temp_metadata <- all_multi_seurat_object[[]] %>%
  rownames_to_column("Cell") %>%
  mutate(SampleGroup = str_remove(orig.ident, "-.*")) %>% 
  mutate(SampleName = orig.ident) %>%
  column_to_rownames("Cell")

all_multi_seurat_object[[]] <- temp_metadata
head(all_multi_seurat_object[[]])
unique(all_multi_seurat_object[["SampleName"]])

# Add percentage of UMIs mapped to mitochondrial genes to the metadata
all_multi_seurat_object[["percent.mt"]] <- PercentageFeatureSet(all_multi_seurat_object,
                                                      pattern = "^MT-")
...
VlnPlot(all_multi_seurat_object, 
...
all_metadata <- all_multi_seurat_object[[]]

# Apply the filtering at the SampleGroup level
# YOUR CODE HERE
sample_name_keep_cells <- all_metadata %>%
  rownames_to_column("Cell") %>%
  group_by(SampleName) %>%
  filter(nFeature_RNA > (median(nFeature_RNA) - 2 * mad(nFeature_RNA)),
         nCount_RNA > (median(nCount_RNA) - 2 * mad(nCount_RNA)),
...
all_multi_seurat_object[[]] <- new_metadata
